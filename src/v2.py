...
def get_config():
    ...
    config = {
        ...
        "github": {
            ...
            "releases_uri": "/repos/{repo}/releases"
        },
        "features": {
...
def get_semver_version(git_tag=None):

    if git_tag is None:
        return semver.VersionInfo.parse("0.0.0")

    return semver.VersionInfo.parse(git_tag.replace("rc/", "").replace("v", ""))

def get_new_semver_version(tag_last, bump_type):
...
def main():
    ...
    if active_branch == config["primary_branch"]:
        ...
        new_semver_version = get_new_semver_version(tag_last, bump_type)
        new_tag = f"{config['tag_prefix']['candidate']}{str(new_semver_version)}"
        logging.info(f"New tag: {new_tag}")
        push_git_tag(repo, new_tag)

        if (active_branch in config["auto_release_branches"]) or ('[RELEASE]' in commit_message and active_branch == config["primary_branch"]):
            ...
            repo.git.commit('--allow-empty', '-m', f"[semver-action] Bump upsteam version up to {str(new_semver_version_after_release)}")
